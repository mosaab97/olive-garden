...
export const createOrder = createAsyncThunk('orders/create', async (data, { rejectWithValue }) => {
  try { return await ordersService.create(data); }
...
export const fetchMyOrders = createAsyncThunk('orders/fetchMine', async (_, { rejectWithValue }) => {
  try { return await ordersService.getMyOrders(); }
  catch (err) { return rejectWithValue(err.response?.data?.message || 'Failed to load orders'); }
});

export const fetchOrderById = createAsyncThunk('orders/fetchById', async (id, { rejectWithValue }) => {
  try { return await ordersService.getById(id); }
  catch (err) { return rejectWithValue(err.response?.data?.message || 'Order not found'); }
});
...
export const updateOrderStatus = createAsyncThunk('orders/updateStatus', async ({ id, status }, { rejectWithValue }) => {
  try { return await ordersService.updateStatus(id, status); }
  catch (err) { return rejectWithValue(err.response?.data?.message || 'Failed to update order'); }
});

export const cancelOrder = createAsyncThunk('orders/cancel', async (id, { rejectWithValue }) => {
  try { return await ordersService.cancelOrder(id); }
  catch (err) { return rejectWithValue(err.response?.data?.message || 'Failed to cancel order'); }
});

export const addTracking = createAsyncThunk('orders/addTracking', async ({ id, tracking_number }, { rejectWithValue }) => {
  try { return await ordersService.addTracking(id, tracking_number); }
  catch (err) { return rejectWithValue(err.response?.data?.message || 'Failed to add tracking'); }
});

// Helper — replaces or inserts an order in the list
const upsertOrder = (items, updated) => {
  const idx = items.findIndex((o) => o.id === updated.id);
  if (idx !== -1) { const next = [...items]; next[idx] = updated; return next; }
  return items;
};
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchOrderById.fulfilled, (state, action) => {
        state.loading      = false;
        state.currentOrder = action.payload;
        state.items        = upsertOrder(state.items, action.payload);
      })
